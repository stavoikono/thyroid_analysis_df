...
-   `step_impute_bag()` for imputing the missing data using bagged tree
...
-   `step_normalize()` scale and center all numerica values
...
tune_spec <- rand_forest(mtry = tune(), min_n = tune(), trees = 100) %>%
  set_engine("ranger") %>%
...
- plain fit with `r fit()` function
- fitting with resample using the `r fit_resamples()` function
- if we want to tune the hyperparameters `r tune_grid()`
...
asdf %>% collect_predictions() %>% roc_auc(Thyroid_Class, .pred_negative:.pred_sick) #results on test set
